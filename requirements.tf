...
resource "null_resource" "check_requirements" {
  provisioner "local-exec" {
    command = <<EOF
      powershell -command "Start-Process -FilePath 'powershell' -ArgumentList '-Command if (Get-Command choco -ErrorAction SilentlyContinue) { Set-Variable -Name TF_VAR_choco_installed -Value true -Scope Global }' -Verb RunAs"
      powershell -command "Start-Process -FilePath 'powershell' -ArgumentList '-Command if (Get-Command terraform -ErrorAction SilentlyContinue) { Set-Variable -Name TF_VAR_terraform_installed -Value true -Scope Global }' -Verb RunAs"
      powershell -command "Start-Process -FilePath 'powershell' -ArgumentList '-Command if (Get-Command docker -ErrorAction SilentlyContinue) { Set-Variable -Name TF_VAR_docker_desktop_installed -Value true -Scope Global }' -Verb RunAs"
    EOF
  }
}
